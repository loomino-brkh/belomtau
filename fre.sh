...
EOL
    ...
    if [ ! -f "$PROJECT_DIR/gunicorn.sh" ] || ! grep -q "exec gunicorn --reload --log-level=debug --workers 5 --bind 0.0.0.0:8900 $APP_NAME.wsgi:application" "$PROJECT_DIR/gunicorn.sh"; then
